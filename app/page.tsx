...
export default function Page() {
  ...
  return <main>
    ...
    <section className="quote-section reveal"><div className="quote-image"><img src={ceo} alt="Portrait of a creative leader" /></div><div className="quote-copy"><span className="eyebrow">A few nice words</span><blockquote>{quotes[quote][0]}</blockquote><p className="quote-by">{quotes[quote][1]}<br /><span>{quotes[quote][2]}</span></p><div className="quote-controls"><button onClick={() => setQuote((quote + quotes.length - 1) % quotes.length)} aria-label="Previous quote">←</button><span>0{quote + 1} / 0{quotes.length}</span><button onClick={() => setQuote((quote + 1) % quotes.length)} aria-label="Next quote">→</button></div></div></section>

    <section className="numbers reveal"><div className="section-label">04 / By the numbers</div><div className="numbers-grid"><div><strong>17</strong><span>Brands launched</span></div><div><strong>09</strong><span>Countries reached</span></div><div><strong>∞</strong><span>Curiosity levels</span></div></div></section>
